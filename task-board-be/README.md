...
`PORT` must be an integer between 1 and 65535. Keep
`TASK_BOARD_COOKIE_SECURE=true` for production HTTPS; set it to `false` only
for local or test HTTP.

The MCP stdio adapter is available through:
...
It registers `task_board_whoami`, `task_board_list_tasks`, `task_board_get_task`, `task_board_create_task`, `task_board_update_task`, and `task_board_move_task`. It retains the session cookie only in process memory and never writes it to output.
